  ...
  return (
    <div className="flex justify-center">
      <div className="px-4" style={{ maxWidth: "1600px" }}>
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-20 pt-4 mt-10">
          {nfts.map((nft, i) => (
            <div
              key={i}
              className="border shadow rounded-xl overflow-hidden min-h-full"
            >
              <img
                src={nft.image}
                alt="picture"
                style={{ height: "192px" }}
                className="w-max object-cover self-center ml-auto mr-auto"
              />
              <div className="p-4">
                <p
                  style={{ height: "64px" }}
                  className="text-2xl font-semibold border-t-2 solid"
                >
                  {nft.name}
                </p>
